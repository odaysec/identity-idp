...
RSpec.describe Encryption::MultiRegionKmsProfileMigrator do
  ...
  def encrypt_single_region_pii(profile, pii, password)
    pii_json = pii.to_json
    pii_encryptor = Encryption::Encryptors::PiiEncryptor.new(password)
    profile.encrypted_pii = pii_encryptor.encrypt(pii_json, user_uuid: profile.user.uuid)
  end

  def encrypt_single_region_recovery_pii(profile, pii, password)
    pii_json = pii.to_json
    pii_encryptor = Encryption::Encryptors::PiiEncryptor.new(password)
    profile.encrypted_pii_recovery = pii_encryptor.encrypt(pii_json, user_uuid: profile.user.uuid)
  end
end
